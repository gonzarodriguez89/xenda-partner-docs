...
| `listo` | Tu POS | Terminado, esperando retiro o despacho |
...
| `asignado`, `en_camino` | Xenda | Solo pedidos con delivery gestionado por Xenda. Tu POS no los emite, pero puede verlos al consultar |
...
- **Transiciones válidas**: `aceptado`/`rechazado` solo desde `pendiente`; `listo`
  desde `en_preparacion`; `entregado` desde cualquier estado en curso; `cancelado`
